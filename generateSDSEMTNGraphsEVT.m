function generateSDSEMTNGraphsEVT(graphName,processedSoraArr, eventsToAnnotate,spreadMeasure,nPoints,lineColourMap, shadingColourMap)
    ...
    for i = 1:nOfTrends
        currData = processedSoraArr(i);
        plot(normTCell{i}, currData.X, 'k','LineWidth',2,'Color',lineColourMat(i,:))
        
        switch spreadMeasure
            case "SD"
                plot(normTCell{i}, currData.X+currData.Xstd, 'k--','LineWidth',1,'Color',lineColourMat(i,:))
                plot(normTCell{i}, currData.X-currData.Xstd, 'k--','LineWidth',1,'Color',lineColourMat(i,:))
            case "SEM"
                plot(normTCell{i}, currData.X+currData.Xsem, 'k--','LineWidth',1,'Color',lineColourMat(i,:))
                plot(normTCell{i}, currData.X-currData.Xsem, 'k--','LineWidth',1,'Color',lineColourMat(i,:))
        end
    end
